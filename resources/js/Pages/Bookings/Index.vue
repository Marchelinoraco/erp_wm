...
async function revertToPending(booking) {
    if (!(await confirm({ title: 'Kembalikan ke "Belum di-booking"?', description: 'Tagihan supplier yang belum dibayar akan dihapus dari Keuangan.', confirmLabel: 'Kembalikan', destructive: false }))) return
    router.patch(route('bookings.update', booking.id), { status: 'pending' }, { preserveScroll: true })
}
async function cancelBooking(booking) {
    if (!(await confirm({ title: 'Tandai booking ini BATAL?', description: 'Tagihan supplier yang belum dibayar akan dihapus dari Keuangan.', confirmLabel: 'Batalkan booking' }))) return
    router.patch(route('bookings.update', booking.id), { status: 'cancelled' }, { preserveScroll: true })
}
async function deleteBooking(booking) {
    if (!(await confirm({ title: 'Hapus baris booking ini?', description: 'Baris booking akan dihapus permanen.', confirmLabel: 'Hapus' }))) return
    ...
    <AuthenticatedLayout>
        ...
        <div class="py-6">
            <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 space-y-6">
                ...
                <!-- Empty -->
                <div v-if="tours.length === 0" class="rounded-lg border bg-white px-4 py-16 text-center shadow-sm">
                    ...
                    <p class="text-sm text-muted-foreground">Belum ada tour <strong>confirmed</strong>. Booking otomatis muncul saat inquiry dikonfirmasi.</p>
                </div>
