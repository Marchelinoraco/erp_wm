...
| `cost_requests` | `belongsTo Tour`, `belongsTo Supplier`, `belongsTo Bill`, `belongsTo Invoice`, `belongsTo User` (requested_by/reviewed_by) | Field `invoice_id` hanya terisi kalau `bill_customer` dicentang saat approve; lihat detail penuh di [01-penjualan-tour.md](01-penjualan-tour.md) |
| `invoices` | (lihat 01) | Di Keuangan selalu difilter `scopeApproved()` |
...
- Rincian Profit yang ditampilkan di panel AR bersifat **read-only** bagi akuntan (tidak ada form edit di `Finance/Tour.vue`) — kalau item Rincian Profit perlu dikoreksi setelah approve, itu di luar kewenangan akuntan lewat halaman ini (item invoice terkunci pasca-approve, lihat [01-penjualan-tour.md](01-penjualan-tour.md) §3).
